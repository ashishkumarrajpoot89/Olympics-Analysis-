...
def data_over_time(df, col):
    temp_df = df.drop_duplicates(subset=['Year', col])['Year'].value_counts().reset_index()
    temp_df.columns = ['Year', 'Count']  # Rename columns explicitly
    temp_df = temp_df.sort_values('Year')  # Sort by 'Year'
    temp_df.rename(columns={'Year': 'Edition', 'Count': col}, inplace=True)
    return temp_df
...
def most_successful(df, sport):
    ...
    # Get the top 15 most successful athletes and rename columns explicitly
    x = temp_df['Name'].value_counts().reset_index().head(15)
    x.columns = ['Name', 'Medals']  # Rename columns for clarity

    # Merge to get additional details like Sport and Region
...
def most_successful_countrywise(df, country):
    ...
    # Count the occurrences of names and create a DataFrame
    x = temp_df['Name'].value_counts().reset_index().head(10)
    x.columns = ['Name', 'Medals']  # Rename columns for clarity

    # Merge to include additional information
